...
from analitica.models import CurvasBase, CurvasBaseHH, CurvasBaseValor
...
class IndexAnalitica(ProyectoMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        #Funciones curva s valor monetario
        reporte_curva_s_avance_real_valor = self.reporte_curva_s_avance_real_valor()
        reporte_curva_s_avance_esperado_valor = self.reporte_curva_s_avance_esperado_valor()       
        reporte_curva_s_fechas_valor = self.reporte_curva_s_fechas_valor()
        ...
        #Curva base valor monetario
        qs_valor_monetario = CurvasBaseValor.objects.filter(proyecto=self.proyecto).last()
        context['CurvasBaseValor'] = qs_valor_monetario
    ...
    def Obtener_fechas_valor(self):
        ...
        documentos = self.get_queryset()
        valor_ganado = len(documentos)
        curva_base = CurvasBaseValor.objects.filter(proyecto=self.proyecto).last()
    ...
    def reporte_curva_s_avance_real_valor(self):

        documentos = self.get_queryset()
        valor_ganado = len(documentos)
        total_valor_monetario = 0
        lista_final = self.Obtener_fechas_valor()
        dia_actual = timezone.now()
        dia_actual = dia_actual.replace(tzinfo = None)
        versiones_documentos = self.get_versiones()
        rev_letra = self.proyecto.rev_letra
        ...
        if valor_ganado !=0:
            ...
            for doc in documentos:
                ...
                #Calculo del total de HH
                total_valor_monetario = total_valor_monetario + doc.valor_monetario

            if total_valor_monetario != 0:

                #Se recorren las versiones a calcular el avance real
                for docs in lista_versiones:
                    contador_avance = 0
                    valor_monetario_doc = docs[0].valor_monetario
                                ...
                                for controles in fechas_controles_recorrer:
                                    ...
                                    if validacion == 0:
                                        if valor_documento == 0:
                                            ...
                                            if contador_avance == 0:
                                                if fecha_version <= controles:
                                                    avance_documento = float((valor_monetario_doc * 100)/total_valor_monetario)
                                                if cont == (len(fechas_controles) - 1):
                                                    if fecha_version > controles:                                
                                                        avance_documento = float((valor_monetario_doc * 100)/total_valor_monetario)
                                            ...
                                            if contador_avance != 0:
                                                if fecha_version <= controles:
                                                    avance_documento = float((valor_monetario_doc * float(100 - rev_letra))/total_valor_monetario)
                                                if cont == (len(fechas_controles) - 1):
                                                    if fecha_version > controles:                                
                                                        avance_documento = float((valor_monetario_doc * float(100 - rev_letra))/total_valor_monetario)
                                ...
                                for controles in fechas_controles_recorrer:
                                    ...
                                    if validacion == 0:
                                        if valor_documento == 0:
                                            ...
                                            for revision in TYPES_REVISION[2:5]:
                                                if revision[0] == revision_documento and fecha_version <= controles:
                                                    calculo_real_b = float((valor_monetario_doc * rev_letra)/total_valor_monetario)
                                                if cont == (len(fechas_controles) - 1):
                                                    if revision[0] == revision_documento and fecha_version > controles:                              
                                                        calculo_real_b = float((valor_monetario_doc * rev_letra)/total_valor_monetario)
                                            ...
                                            if contador_avance == 0:
                                                ...
                                                for revision in TYPES_REVISION[6:]:
                                                    if revision[0] == revision_documento and fecha_version <= controles:
                                                        calculo_real_0 = float((valor_monetario_doc * 100)/total_valor_monetario)
                                                    if cont == (len(fechas_controles) - 1):
                                                        if revision[0] == revision_documento and fecha_version > controles:                                
                                                            calculo_real_0 = float((valor_monetario_doc * 100)/total_valor_monetario)
                                            ...
                                            if contador_avance != 0:
                                                ...
                                                for revision in TYPES_REVISION[6:]:
                                                    if revision[0] == revision_documento and fecha_version <= controles:
                                                        calculo_real_0 = float((valor_monetario_doc * float(100 - rev_letra))/total_valor_monetario)
                                                    if cont == (len(fechas_controles) - 1):
                                                        if revision[0] == revision_documento and fecha_version > controles:                                
                                                            calculo_real_0 = float((valor_monetario_doc * float(100 - rev_letra))/total_valor_monetario)
            ...
            if total_valor_monetario == 0:
                avance_inicial = []
                avance_final = []
                avance_inicial = [total_valor_monetario]
                avance_final.append(avance_inicial)
    ...
    def reporte_curva_s_avance_esperado_valor(self):
        ...
        if valor_ganado != 0:
                ...
                for controles in fechas_controles:
                    if contador_largo < len(fechas_controles):
                        ...
                        for doc in documentos:                  
                            fecha_emision_b = doc.fecha_Emision_B.replace(tzinfo=None)
                            fecha_emision_0 = doc.fecha_Emision_0.replace(tzinfo=None)
                            valor_monetario_doc = doc.valor_monetario

                            #Se calcula el avance esperado mediante la comparación de la fecha de control y la fecha de emisión en B - 0
                            if fecha_emision_b <= controles and fecha_emision_0 > controles:
                                calculo_avanceEsperado = float((valor_monetario_doc * rev_letra)/total_valor_monetario) + calculo_avanceEsperado                      
                            if fecha_emision_0 <= controles and fecha_emision_b < controles:
                                calculo_avanceEsperado = float((valor_monetario_doc * 100)/total_valor_monetario) + calculo_avanceEsperado
            ...
            if total_valor_monetario == 0:
                avance_esperado = [int(total_valor_monetario)]
                lista_final_esperado.append(avance_esperado)
    ...
    def reporte_curva_s_fechas_valor(self):
        
        lista_final = self.Obtener_fechas_valor()
        valor_ganado = self.get_queryset().count()
        lista_avance_real = self.reporte_curva_s_avance_real_valor()
        fechas_controles = lista_final[0][0]
        diferencia = 0
        contador = 0
        ultima_fecha = 0
...
class CurvaBaseValorView(ProyectoMixin, TemplateView):
                ...
                for controles in fechas_controles:
                    if contador_largo < len(fechas_controles):
                        ...
                        for doc in documentos:                  
                            fecha_emision_b = doc.fecha_Emision_B
                            fecha_emision_0 = doc.fecha_Emision_0
                            valor_doc = doc.valor_monetario

                            #Se calcula el avance esperado mediante la comparación de la fecha de control y la fecha de emisión en B - 0
                            if fecha_emision_b <= controles and fecha_emision_0 > controles:
                                calculo_avanceEsperado = float((valor_doc * rev_letra)/total_valor_monetario) + calculo_avanceEsperado                      
                            if fecha_emision_0 <= controles and fecha_emision_b < controles:
                                calculo_avanceEsperado = float((valor_doc * 100)/total_valor_monetario) + calculo_avanceEsperado
                    contador_largo = contador_largo + 1
    ...
    def get_queryset(self, request, *args, **kwargs):
        qs = CurvasBaseValor.objects.filter(proyecto=self.proyecto)
        return qs
    ...
    def get(self, request, *args, **kwargs):
        context = self.get_context_data(**kwargs)
        qs = CurvasBaseValor.objects.filter(proyecto=self.proyecto).last()
        context['CurvasBaseValor'] = qs
        return self.render_to_response(context)  
